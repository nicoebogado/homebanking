...
HScript::register([
    'commons/cypher.min',
    'vendor/jquery.inputmask/jquery.inputmask.bundle.min',
    'vendor/jquery-validation/dist/jquery.validate',
    'vendor/jquery.steps/build/jquery.steps.min',
]);
        ...
        foreach ($wizardOptions as $opt) {
            ...
            if($aux==3){
                if(isset($frequentAccounts) && count($frequentAccounts)>0){
                    ...
                    <input type="hidden" id="sipap_transfers" value="true" />
                    <table class="items table table-striped table-condensed table-hover">
                    <thead>
                        <tr>
                            <td>'.Yii::t('sipapTransfer','Seleccionar').'</td>
                    ...
                    foreach ($frequentAccounts as $account) {
                        $h .= '<tr>
                            ...
                            <td>'.$account['cuentabeneficiario'].'</td>
                            <td>'.$account['tipodocbeneficiario'].'</td>
                            <td>'.$account['numerodocbeneficario'].'</td>
                            <td>'.$account['nombrebeneficiario'].'</td>
                            <td>'.$account['direccionbeneficiario'].'</td>
