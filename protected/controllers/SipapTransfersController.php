...
class SipapTransfersController extends Controller
{
    ...
    public function actionTransfer($id = null)
    {
        ...
        /*
        *Added:01-04-2022, Higinio Samaniego: Se muestra solo cuentas guaranies en form de transferencias ACH
        */
        if($id == 'B'){
            foreach($gridOptions['datas'] as $datosCuenta){  
                if($datosCuenta['currency'] == 'USD'){
                    unset($accountOptions[$datosCuenta['hash']]);
                }   
            }
        }
        ...
        $wizardOptions = array(
            ...
            array(
                ...
                'elements' => array('currency', 'amount', 'date', 'reason', 'concept', 'hasAgreement', 'exchangeContract', 'creditQuotation'),
            ),
            array(
    ...
    public function actionVerify()
    {
        ...
        if (isset($_POST['TransferForm'])) {
            ...
            if ($_POST['TransferForm']['actionType'] === 'verificar') {
                ...
                if ($model->validate() || isset($_POST['TransferForm']['isToken'])) {
        ...
        try {
            ...
            if(isset($codigoempresa)){
                if(trim($perfil) == 'AUTORIZA' || trim($perfil) == 'COMPLETO'){
                    ...
                    if($verifyTransfer->error === 'N' && $verifyTransfer->nombrerespuesta === '273'){
                    ...
                    if ($verifyTransfer->error === 'S') {
                        ...
                        //Creo que al pedo esta esto con token fisico
                        //$form = new TbForm($model->hiddenFormConfig(), $model);
                      
                        $this->render('transfers/confirmtoken', array(
                        ...
                        if ($verifyTransfer->error === 'S') {
                            ...
                            $model->scenario = 'confirm';
                            $model->actionType = 'confirmar';
                            $mensajeRegistra = 'Operación registrada con éxito';
                            $this->_confirmTransaction($model, $mensajeRegistra);
                        }   

                }else if($perfil == 'CONSULTA'){//No posee permisos cuando es perfil consulta en banca empresas
                    $this->setFlashError("El perfil no posee priviliegios para la operación requerida.");
                    $this->redirect(array('sipapTransfers/transfer/'));
                }
    ...
    private function _confirmTransaction($model ,$mensajeFlash = null)
    {
        try {
            $verifyTransfer = $this->wsClient->snpTransfer(array(
                "mode"                      => 'C',
            ...
            $perfilAuto = Yii::app()->user->getState('clientPerfil');
            ...
            if ($verifyTransfer->error === 'S') {
                ...
                if(isset($mensajeFlash)){
                    $this->setFlashSuccess($mensajeFlash);
                }else if($verifyTransfer->autorizacionesfaltantes != 0 && $perfilAuto == 'AUTORIZA'){    
                    $this->setFlashSuccess('Operación realizada con éxito, autorizaciones faltante(s): '.$verifyTransfer->autorizacionesfaltantes);
                }else{
            ...
            if(isset($codigoempresa)){
                if(trim($perfil) == 'AUTORIZA' || trim($perfil) == 'COMPLETO'){
                    ...
                    if ($response->error === 'S') {
                        ...
                        $data = $data->array[0];
                        $this->render('transfers/cancellationsToken', array(
                            'data' => $data,
                            'stage' => 'detail',
                        ));
                    }
    ...
    public function actionVoucher($id = null, $titulo = "", $nombreremitente = "")
    {
        try {
            $response = $this->wsClient->detailSnp(array(
                'referenceNumber' => $id,
            ...
            //para saber si es banca empresa
            $codigoempresa = Yii::app()->user->getState('codigoempresa') ? Yii::app()->user->getState('codigoempresa') : null;
            $entityName = Yii::app()->user->getState('entityName') ? Yii::app()->user->getState('entityName') : null;
