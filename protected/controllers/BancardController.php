...
{
    /**
     * Lista de ids de marcas que no se deben mostrar
     * como opciones de pagos
     */
    protected $blacklist = [
        14, // Coomecipar
        216, // BNF
    ...
    public function actionList()
    {
        ...
        if ($brands->status === 'success') {
            $brands = $this->_parseBrandList($brands);
    ...
    {
		/******* VALIDAR ACA CON LA OTP DE JAVIER *****/
     ...
     */
    private function _parseBrandList($brandList)
    {
        $categoryLabels = [
        ...
        $brandList->categoryLabels = $categoryLabels;

        // por cada brand
        foreach ($brandList->brands as $brand) {
        ...
        // filtrar maracas que estén en el blacklist
        $brands = $brandList->brands;

        $brandList->brands = array_filter($brands, function ($e) {
            return !in_array($e->id, $this->blacklist);
        });
