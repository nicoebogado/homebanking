...
Yii::import('ext.pdf.fpdf.PDF_Extract_Mov');

class EPDF_Extract_Mov extends CWidget
{
	...
	public function init()
	{
		...
		$this->_pdf = new PDF_Extract_Mov('P', 'mm', $config['pdfSize']);
	...
	protected function renderTableHeader()
	{
		if (!$this->hideHeader) {
			// titulo de la tabla
			$this->_pdf->SetBold();
			//$this->_pdf->Ln();//comentado para reducir espacio entre cabeceras
	...
	protected function _calcWidths($params)
	{
		...
		if (!$params) {
			...
			if($this->tableTitle == "Resumen de cuenta"){
				$w = $this->_pdf->tableWidth / $visibleCols;
				for ($i = 0; $i < $visibleCols; $i++){
					//$widths[] = $w;
					if($i == 3){//ancho para concepto
						$widths[] = 28;
					}elseif($i == 0  ){
						$widths[] = 40;
					}elseif($i == 1){
